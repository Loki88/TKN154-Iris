...
module TKN154RadioP {
	...
	provides {
	    ...
	    interface SetNow<uint8_t> as RadioPower;
	    interface SetNow<bool> as SetOff;
...
// #define RX_BUFF_SIZE 4
	...
	async command error_t RadioOff.off() {
		...
		atomic {
			...
			else if (m_state != S_RECEIVING || m_sync == TRUE) // it isn't possible to stop during tx
				return FAIL;
	...
	async command error_t RadioTx.transmit(ieee154_txframe_t *frame, uint32_t t0, uint32_t dt) {
		...
		result = turnOn(); // radio can be used to transmit once RadioSend.ready is signaled
		if(result == SUCCESS || result == EALREADY){
			if(result == EALREADY) // radio is yet on
				m_sync = TRUE;
			if(m_dt == 0 || call TimeCalc.hasExpired(m_t0, m_dt))
	...
	async command error_t SlottedCsmaCa.transmit(ieee154_txframe_t *frame, ieee154_csma_t *csma,
		...
		} else if (result != SUCCESS){ // radio is busy and it is off
			atomic{
				m_state = S_RADIO_OFF;
			}
			return FAIL;
